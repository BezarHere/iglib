...
GL_VIEWPORT
When used with non-indexed variants of glGet (such as glGetIntegerv), data returns four values: the x
 and y
 window coordinates of the viewport, followed by its width and height. Initially the x
 and y
...
When used with indexed variants of glGet (such as glGetIntegeri_v), data returns four values: the x
 and y
 window coordinates of the indexed viewport, followed by its width and height. Initially the x
 and y
...
regex = re.compile(r"\n(GL_\w+)\s+", re.IGNORECASE)
gl_pnames: set[str] | list[str] = set()

for i in regex.finditer(src):
	gl_pnames.add(i[1])

gl_pnames = list(gl_pnames)
gl_pnames = sorted(gl_pnames)
print(f"there is {len(gl_pnames)} pname for OpenGL")
...
for i in gl_pnames:
	field_names.append(Field(i[3:].lower(), "GLStateField"))
...
print("\n{")
for i in gl_pnames:
	print("  { { " + f'"{i[3:].lower()}"' + " }, " + f"{i} " + "},")
print("}\n")


for i in field_names:
	print(f"load_state_field({'GL_' + i.name.upper()}, states.{i.name});")
